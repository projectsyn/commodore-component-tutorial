...
Redirect trafic to ArgoCD:

`kubectl --context minikube -n syn port-forward svc/argocd-server 8080:80`

then open http://localhost:8080 username `admin` and get the password as follows:

`kubectl --context minikube -n syn get secret/steward -o jsonpath='{.data.token}'  | base64 --decode | pbcopy`

=== Test the fortune-service running:

TODO: Add this section

k3d: `kubectl --context k3d-projectsyn -n syn-fortune port-forward svc/fortune-service 8081:3000`

`curl http://localhost:8081`

minikube: `minikube service fortune-service --namespace syn-fortune`
