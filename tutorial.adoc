...
The second step in this tutorial is to bootstrap https://docs.syn.tools/commodore/[Commodore] in our Minikube cluster. For that we can just run the `./2_commodore_on_minikube.sh` script.

The `./2_commodore_on_minikube.sh` script requires an environment variable to run: `COMMODORE_SSH_PRIVATE_KEY` should contain the path to your personal SSH public key.
...
You can access the ArgoCD console in both clusters through these steps:
