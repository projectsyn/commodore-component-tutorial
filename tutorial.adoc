...
The second step in this tutorial is to bootstrap https://docs.syn.tools/commodore/[Commodore] in our Minikube cluster. For that we can just run the `./2_commodore.sh` script.

The `./2_commodore.sh` script requires an environment variable to run: `COMMODORE_SSH_PRIVATE_KEY` should contain the path to your personal SSH public key.
...
You can access the ArgoCD installation in your cluster by following these steps:
...
TIP: In Linux, you can `alias pbcopy="xclip -selection clipboard"``

Second, redirect trafic to ArgoCD:

`kubectl --context minikube -n syn port-forward svc/argocd-server 8080:80`

And then open http://localhost:8080 with the username `admin` and the password you copied in the previous step.


TODO: image of ArgoCD
...
* Remove your GitLab projects: `mytenant` and `cluster-gitops1`
