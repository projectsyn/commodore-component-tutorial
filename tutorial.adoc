...
* As an editor, for this tutorial we will be using https://code.visualstudio.com/[Visual Studio Code] with the following extensions:
...
----
# ...
include::1_lieutenant_on_minikube.sh[tag=demo]
# ...
----
...
In order to create a new component, we need to clone the official Commodore project from GitHub first. Inside of that clone of the Commodore source code we will create a few folders:
...
We also need to define the contents of the `.env` file, used by Docker to set environment variables required by the `commodore` command. We can do this in a snap with this command, which should display an output similar to this one:
...
COMMODORE_GLOBAL_GIT_BASE=ssh://git@github.com/$GITHUB_USERNAME
----
...
The Jsonnet extension for Visual Studio Code requires some values in your `settings.json` in order for the preview pane to work properly:
...
The `global` folder is a clone of the fork of the `commodore-defaults` project.
...
Commit and push your changes, and `cd` to another folder:
...
We're ready! Now we just have to recompile both catalogs and push the changes.
...
We have to do this for both clusters!
...
TODO: Add K9s screenshot showing the `fortune-service` in both clusters.
