...
. In GitHub, fork the https://github.com/projectsyn/getting-started-commodore-defaults[getting-started-commodore-defaults] project to your own personal account.
...
You can also synthesize a generic, pre-existing cluster using the `./4_synthesize_on_k8s.sh` script. This requires a file named `demo.kubeconfig` to be present, with the required accesses for `kubectl` (or you can edit the script and reference your context instead.)
...
The `global` folder is a clone of the fork of the `getting-started-commodore-defaults` project.
...
* Remove your clone of `getting-started-commodore-defaults` on GitHub.
