...
CLUSTER_ID=$(curl -s -H "$LIEUTENANT_AUTH" -H "Content-Type: application/json" -X POST --data "{ \"tenant\": \"${TENANT_ID}\", \"displayName\": \"Generic Kubernetes cluster\", \"facts\": { \"cloud\": \"exoscale\", \"distribution\": \"k3s\", \"region\": \"ch-gva-2\" }, \"gitRepo\": { \"url\": \"ssh://git@${GITLAB_ENDPOINT}/${GITLAB_USERNAME}/tutorial-cluster-k8s.git\" } }" "${LIEUTENANT_URL}/clusters" | jq -r ".id")
...
echo "===> Install Steward in the Kubernetes cluster"
kubectl --kubeconfig demo.kubeconfig apply -f "$STEWARD_INSTALL"

echo "===> Check that Steward is running and that Argo CD Pods are appearing"
kubectl --kubeconfig demo.kubeconfig -n syn get pod
